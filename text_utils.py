...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        with open(char_freq_path,'rb') as f:
            b_char_freq = cp.load(f, encoding="bytes")
            self.char_freq = { key.decode(): val for key, val in b_char_freq.items() }

        # get the model to convert from pixel to font pt size:
        with open(font_model_path,'rb') as f:
            b_font_model = cp.load(f, encoding="bytes")
            self.font_model = { key.decode(): val for key, val in b_font_model.items() }
